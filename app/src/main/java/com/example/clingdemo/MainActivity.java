        ...
        public void deviceAdded(final Device device) {
            runOnUiThread(new Runnable() {
                public void run() {
                    ...
                    int position = clingDialog.getPosition(d);
//                    if (position >= 0) {
//                        // Device already in the list, re-set new value at same position
//                        clingDialog.remove(d);
//                        clingDialog.insert(d, position);
//                    } else {
                    clingDialog.add(d);
//                    }
                }
    ...
    public void touPing(View view) {
        clingDialog = ClingDialog.newInstance(new ChoiceDeviceListener() {
            ...
            public void choiceDevice(Device device) {
                ...
                long size = 0;
                Res res = new Res(new MimeType(ProtocolInfo.WILDCARD, ProtocolInfo.WILDCARD), size, source1);
...
//                });
        ...
        clingDialog.show(getSupportFragmentManager());
